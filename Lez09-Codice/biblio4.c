...
void StampaIstruzioniRiordino (listalibri *pResi, listalibri *pScaffale)
{
  ...
  while (!listavuota(*pResi))
  {
    ...
    /* Cerca la posizione corretta per il libro (primo libro successivo) */
    for (q = primolista(*pScaffale); finelista(*pScaffale,q) == FALSE; q = succlista(*pScaffale,q))
    {
      leggelista(*pScaffale,q,s);
      if (strcmp(s,AutoreTitolo) > 0) break;
    }

    /* Inserisce il libro prima della posizione trovata, stampando messaggi diversi se e' l'ultima o una generica */
    if (finelista(*pScaffale,q) == TRUE)
      printf("METTERE %s PER ULTIMO\n",AutoreTitolo);
    else
      printf("METTERE %s PRIMA DI %s\n",AutoreTitolo,s);

    inslista(*pScaffale,q,AutoreTitolo);
